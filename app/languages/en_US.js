...
function en_USLang() {
    ...
    STR_SHOW_ISLIVE_WARNING_SUMMARY = "When watching a clip or a VOD, the app can check when a streamer is live. If this is set to YES, a warning will show. To open the live, just use the player bottom controls option.";
    ...
    STR_DEF_QUALITY_SUMMARY = "This option will always override others when playing a single video. In Picture Picture or multistream mode, the playback needs to use the auto quality. The reason for why that is can be found under the settings option \'" + STR_PLAYER_BITRATE + "\'.";
    ...
    STR_CHAT_LOGGING_SUMMARY = "The app will always log to chat using the current user when an authorization key is provided, unless chat is disabled on player bottom controls, but if this option if set to NO, it will prevent logging using current username, and will instead log as anonymous, even with a provided authorization key. This doesn\'t prevent from sending chat messages for this user, if a key was added, but does prevent from knowing if you are banned in the chat and prevents from seeing the chat\'s ROOMSTATE.";
