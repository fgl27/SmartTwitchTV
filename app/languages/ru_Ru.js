...
function ru_RULang() {
    ...
    STR_UPDATE_SHOW = "Показывать диалог обновлений, когда обновления доступны";
    ...
    STR_USER_NUMBER_ONE = "Первый пользователь может следить (при предоставлении ключа) и видеть поток каналов в реальном времени за пределами экрана пользователя.<br>";
    STR_ADD_USER_SH = "Добавьте пользователя Twitch, чтобы отображать здесь контент отслеживаемых каналов";
    STR_CLIP_DAY = "24ч";
    STR_CLIP_WEEK = "7д";
    STR_CLIP_MONTH = "30д";
    STR_CLIP_ALL = "все";
    ...
    STR_CONTROLS_MAIN_6 = " Переключить экран: кнопка назад, затем D-Pad вверх/вниз или" + STR_KEY_UP_DOWN + STR_KEY_MEDIA_FF;
    STR_CONTROLS_MAIN_10 = "Начать поиск: на боковой панели щелкните поиск, введите текст, нажмите кнопку ОК на виртуальной клавиатуре и выберите вариант поиска.";
    STR_CONTROLS_MAIN_14 = "Об этом приложении: на боковой панели щелкните значок";
    ...
    STR_CONTROLS_PLAY_0 = STR_SPACE + "или в нижних элементах управления плеером";
    ...
    STR_CONTROLS_PLAY_2 = "Закройте видео: дважды нажмите кнопку назад или кнопку Стоп.";
    STR_CONTROLS_PLAY_3 = "Плей/пауза видео: откройте информационную панель и щелкните символ паузы";
    ...
    STR_CONTROLS_PLAY_8 = "Изменить позицию в чате: D-pad влево, или перемотка назад (только VOD и клипы)" + STR_CONTROLS_PLAY_0;
    ...
    STR_CONTROLS_PLAY_12 = "Начните поиск: откройте инф. панель, перейдите с помощью нав. панели (влево/вправо) в Поиск и нажмите ОК.";
    ...
    STR_SHOW_ISLIVE_WARNING_SUMMARY = "При просмотре клипа или VOD приложение может проверить, активен ли этот стример, если да, появится предупреждение, чтобы открыть прямую трансляцию, просто используйте опцию нижнего элемента управления плеером.";
    STR_OPEN_CHAT = "Нажмите, чтобы открыть чат или дождитесь возвращения в прямой эфир";
    STR_STAY_OPEN = "Остаться на стриме";
    STR_STAY_OPEN_SUMMARY = "Оставайтесь и еще раз проверьте, не онлайн ли он";
    ...
    STR_SWITCH_TYPE = "Сменить: самые последние или просматриваемые";
    STR_ENABLE = "Включить";
    ...
    STR_RESTORE_PLAYBACK_SUMMARY = "Приложение сохраняет то, что играло, на случай непреднамеренного закрытия, при изменении приложений системе может не хватить памяти и закрыть ее или принудительно закрыть пользователем в тех случаях, когда приложение восстановит то, что ранее играло, при следующем запуске.";
    STR_CHAT_FONT = "Размер шрифта чата";
    STR_VIDEOS_ANIMATION = "Анимированные превью видео";
    STR_VIDEOS_ANIMATION_SUMMARY = "Анимировать превью, когда выбран VOD или Хайлайт, если для этого видео доступно анимированное превью (не во всех видео оно есть).";
    ...
    STR_SECOND = " секунд";
    STR_SECONDS = STR_SECOND + "ы";
    STR_MINUTE = " минут";
    STR_MINUTES = STR_MINUTE + "ы";
    STR_CLOCK_OFFSET = "Часы";
    STR_CLOCK_OFFSET_SUMMARY = "Настройте часы приложения в соответствии с вашим выбором";
    ...
    STR_VOD_DIALOG = "Диалог запуска VOD";
    STR_VOD_DIALOG_SUMMARY = 'Выберите поведение по умолчанию, при воспроизведении VOD, если информация о нем присутствует в истории пользователя, его можно воспроизводить с того места, где вы последний раз прекращали просмотр, если для этого параметра установлено значение «всегда с начала», это поведение также будет применяться к предпросмотру VOD';
    ...
    STR_END_DIALOG_SETTINGS_SUMMARY = "Когда Live/VOD/Клип заканчиваются, появляется диалоговое окно с опцией, что делать дальше, установите время (в секундах), которое потребуется для действия по умолчанию.";
    ...
    STR_THUMB_RESOLUTION_SUMMARY = "Разрешение превью по умолчанию для прямых трансляций, видео и игр (не применимо для клипов) более низкое значение поможет приложению загружаться быстрее, но превью может выглядеть размытым.";
    ...
    STR_PLAYER_BITRATE_SUMMARY = 'Это будет использоваться для предотвращения задержек на слабых устройствах при одновременном воспроизведении нескольких видео (большинство устройств будут отставать, пропуская кадры в этой ситуации, поскольку они созданы для воспроизведения только одного видео), а также помогает ограничить использование пропускной полосы Интернета. В случае, если вам нужно ограничение, которое также установит для параметра «Качество проигрывателя по умолчанию» значение «Авто». Рекомендуемое разрешение/битрейт для всех небольших проигрывателей составляет 720p/3 Mbps и неограничен для основного или большого проигрывателя для большинства слабых устройств.';
    ...
    STR_DEF_QUALITY_SUMMARY = 'Этот параметр всегда будет учитываться при воспроизведении одного видео, в режиме Картинка в Картинке или Мультистрим для воспроизведения необходимо использовать качество авто, по этой причине проверьте параметр настроек "' + STR_PLAYER_BITRATE + '"';
    ...
    STR_PICTURE_CONTROLS2 = "Изменение видеоконтента: из предпросмотра плеера, если в многопользовательском режиме всегда один клик, если на КвК или 50/50 одним щелчком мыши обновляется большое или верхнее видео, удерживайте кнопку ОК или нажмите 1, чтобы обновить маленькое или нижнее видео";
    ...
    STR_PICTURE_CONTROLS6 = "Изменение малого размера видео (только картинка в картинке): D-pad вправо";
    STR_PICTURE_CONTROLS7 = "Изменить источник звука: используйте нижние элементы. Источник звука, если в режиме 50/50 или мультистрим режиме, используйте левую/правую кнопки, если на КвК, используйте кнопки мультимедиа следующей/предыдущей дорожки.";
    STR_PICTURE_CONTROLS3 = "Измените источник звука на все видео: при мультистриме или КвК удерживайте кнопку вниз нажатой, на 50/50 одним нажатием вниз";
    ...
    STR_PICTURE_CONTROLS11 = "Закройте маленькое или нижнее видео (только картинка в картинке): дважды нажмите кнопку назад, чтобы выйти из режима КвК или 50/50";
    ...
    STR_NOW_BACKGROUND_SUMMARY = "Если вы запретите уведомление для этого приложения в настройках системы, эта функция не будет работать, если уведомления приложения уже запущены, и вы выйдете из приложения, уведомление будет отображаться поверх других приложений, даже если это отключено.";
    ...
    STR_ROUND_IMAGES_SUMMARY = "Поскольку большинство аватвров каналов имеют квадратную форму, некоторые аватары могут выглядеть некорректно.";
    STR_SCREEN_COUNTER = "Скрыть счетчик позиции/общего количества";
    STR_SCREEN_COUNTER_SUMMARY = "Есть счетчик позиции, который информирует о текущей позиции и общем загруженном контенте на экранах, на которых есть воспроизводимый контент, поскольку вы прокручиваете больше контента, будет загружаться больше контента, и общее количество обновляется.";
    ...
    STR_LIVE_FEED_SORT_SUMMARY = "Сортировка прямых трансляций на боковой панели и превью, применение к пользователю в реальном времени, Ретрансляция (кто ретранслирует) и избранные (все остальные, live и игры - это база просмотров, сначала просматривается история, а vod - самые свежие)";
    STR_A_Z = "Алфавит A - Z";
    STR_Z_A = "Алфавит Z - A";
    STR_APP_ANIMATIONS = "Включить анимации";
    STR_APP_ANIMATIONS_SUMMARY = "Управляет боковой панелью,прокруткой,уведомлениями и связанной анимацией";
    ...
    STR_WARNING = "Предупреждения";
    STR_WARNINGS = "Предупреждени(Я)";
    ...
    STR_CODEC_DIALOG_TITLE = 'Программные кодеки (OMX.google) обычно имеют худшую производительность, но на некоторых устройствах они могут иметь приоритет над аппаратными кодеками, используя это, кодеки можно заблокировать их и посмотреть, улучшится ли производительность воспроизведения, <span style="color: #FF0000;">по умолчанию декодер OMX.google отключен</span> (если доступен другой кодек). Постоянное накопление пропущенных кадров указывает на проблему с кодеком.<br>В контенте приложения используются только декодеры avc/h264, другие не предусмотрены.';
    ...
    STR_MAX_INSTANCES = "Макс. сегменты: ";
    STR_UNKNOWN = "Неизвестно";
    STR_ONE_CODEC_ENA = "Хотя бы один кодек должен быть включен постоянно";
    STR_USER_LIVE = "Боковая панель User Live: на боковой панели D-pad влево или из любого места кнопка 3";
    STR_PP_WORKAROUND = "Временное решение для старой ОС в мультистриме";
    STR_PP_WORKAROUND_SUMMARY = "Для некоторых устройств под управлением Android 7 (Nougat) или старше необходимо включить это для правильной работы многопользовательского режима, не включайте это, если у вас нет проблем, так как это приведет к снижению качества изображения и возможной потере производительности.";
    STR_HISTORY = "История";
    ...
    STR_DELETE_UNREACHABLE_SUMMARY = "Если для этого параметра установлено значение ДА, приложение автоматически удалит VOD и клипы, которые недоступны (были удалены стримером/создателем) из истории.";
    ...
    STR_LIVE_VOD = "Эта прямая трансляция теперь является VOD<br>открытие VOD с того места, где вы в последний раз прекратили смотреть прямую трансляцию:<br>";
    ...
    STR_HISTORY_VOD_DIS = "Включить историю VOD";
    ...
    STR_NO_LIVE_CONTENT = 'Сейчас нет контента, попробуйте еще раз позже';
    ...
    STR_SHOW_VOD_PLAYER = 'Предварительный просмотр на экранах VOD';
    ...
    STR_DISABLED_FEED_PLAYER_MULTI_SUMMARY = 'По соображениям производительности некоторые устройства могут отставать от некоторых плееров, если у вас все в порядке для мультистрима, но когда активны плеер превью и мультистрим, устройство лагает, установите для этого параметра значение НЕТ';
    STR_PREVIEW_ERROR_LOAD = "Превью не загружается:";
    STR_PREVIEW_ERROR_LINK = " недоступно";
    STR_PREVIEW_VOD_DELETED = " , этот VOD мог быть удален";
    ...
    STR_PREVIEW_SIZE_SCREEN_ARRAY = ["Размер миниатюр", "Большой"];
    STR_SIDE_PANEL_PLAYER_DELAY = "Задержка превью";
    STR_SIDE_PANEL_PLAYER_DELAY_SUMMARY = "Установите время задержки, которое займет превью, чтобы начать загрузку после выбора миниатюры, это помогает с медленными устройствами, которые тормозят при прокрутке";
    STR_PREVIEW_VOLUME = "Громкость превью";
    STR_PREVIEW_VOLUME_SUMMARY = "Разрешить для установки, какая будет громкость превью ленты";
    ...
    STR_PP_VOD_ERROR = "Выйдите из КвК или Мультистрима, чтобы открыть этот VOD";
    ...
    STR_ACCESSIBILITY_WARN = " служба(ы) доступности обнаружены";
    STR_ACCESSIBILITY_WARN_EXTRA = "Подробнее читайте по этой ссылке:";
    STR_ACCESSIBILITY_WARN_EXTRA2 = "Если у вас есть проблема с зависанием или задержкой, закройте это приложение и отключите все службы спец. возможностей после того, как все проблемы исчезнут.<br>Чтобы больше никогда не отображать это предупреждение, отключите его в настройках";
    ...
    STR_ENABLED_MAIN_MULTI = "Сначала включите плеер в главном или верхнем левом углу";
    ...
    STR_SOURCE_CHECK_SUMMARY = 'Когда эта опция включена, и вы не используете авто качество плеера, он переключится на авто качество и предупредит об этом. Если плеер не может воспроизвести более 15 секунд, после этого изменения плеер автоматически вернется к источнику, когда вы запустите новый стрим VOD';
    ...
    STR_CONTROLS_MEDIA_FF = "Перемотка (только для VOD и клипов): используйте D-pad вправо/влево или кнопки для быстрой перемотки вперед/назад";
    ...
    STR_ANONYMOUS_USER = "анонимный";
    STR_GIFT_ANONYMOUS = "Ан " + STR_ANONYMOUS_USER;
    ...
    STR_CHAT_NICK_COLOR_SUMMARY = "Вместо использования цвета ников по умолчанию, который иногда не читается на темном фоне, используйте настраиваемый, удобный для чтения цвет.";
    ...
    STR_CHAT_CLEAR_MSG_SUMMARY = 'Удалять сообщения чата от определенного пользователя (обычно после того, как они получили тайм-аут или бан), удаленные сообщения всегда будут иметь синий фон (только если не изменится цвет фона). Сообщение будет удалено, если для этого задано значение «Да»';
