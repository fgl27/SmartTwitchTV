...
function pt_BRLang() {
    ...
    STR_SWITCH_VOD = "Trocar: Videos ou Destaques";
    ...
    STR_SETTINGS_BUFFER_VOD = "Buffer inicial de Vídeos (video e destaque)";
    ...
    STR_VIDEOS_ANIMATION_SUMMARY = "Quando um Video ou destaque é selecionado a miniatura é animada se estiver disponível para aquele vídeo (nem todos os vídeos têm animação)";
    ...
    STR_CONTENT_LANG_SUMMARY = "O idioma do conteúdo das telas, ao vivo, videos, clipes";
    ...
    STR_VOD_DIALOG = "Diálogo inicial de Videos";
    STR_VOD_DIALOG_SUMMARY = 'Escolha o comportamento padrão, ao reproduzir um Video se a sua informação estiver presente no histórico do usuário, ele pode ser reproduzido de onde você parou de assistir pela última vez, se definido como "sempre desde o início" este comportamento também será aplicado a Pré-Visualização de VOD';
    ...
    STR_END_DIALOG_SETTINGS_SUMMARY = "Quando um Live/Video/Clipe termina uma caixa de diálogo mostra opçôes do que fazer a seguir, defina o tempo (em segundos) que levará para a opção padrão agir";
    ...
    STR_PLAYER_PROBLEM_2 = "A conexão falhou, não foi possível carregar as informações do video";
    ...
    STR_PLAYER_BITRATE_SUMMARY = 'Isso será usado para evitar lags em dispositivos low end ao reproduzir vários vídeos ao mesmo tempo (a maioria dos dispositivos vai atrasar pulando frames nessa situação, pois eles são feitos para reproduzir apenas um único vídeo), também ajuda a limitar Use a largura de banda da Internet no caso de você precisar limitar que também defina a "Qualidade do player padrão" para Auto, a resolução/taxa de bits recomendada para todos os players pequenos é 720p/3 Mbps e ilimitada para o player principal ou grande para a maioria dos dispositivos de baixo custo.';
    STR_PLAYER_BITRATE_SUMMARY_ETC = "Valores diferentes aqui para a resolução do player principal e pequeno e/ou taxa de bits, pode causar um curto buffering/carregamento ao mudar com o player principal no modo Picture Picture (pressionando para baixo irá alterar os players), para evitar que isto defina ambos os valores mesmo ao custo de um possível atraso, o melhor indicativo de uma taxa de bits muito alta é um acúmulo constante de quadros ignorados ou um buffer constante do fluxo.";
    STR_PLAYER_MAIN = "player principal, para o player grande em Picture in picture ou o player principal 50/50";
    STR_PLAYER_RES_SMALL = "players pequenos, para o player pequeno do modo Picture in Picture e todos os players Multistream";
    ...
    STR_LIVE_FEED_SORT_SUMMARY = "Ordena o painel lateral e o preview do player, este se aplica apenas ao usuário ao vivo e em destaque (todos os históricos são ordenados por ultimo visto e os videos é o mais recente)";
    ...
    STR_CONTROLS_MULTI_1 = 'Se você está tendo problemas de atraso após habilitar o multistream, tente diminuir o valor de "Taxa de bits do player pequeno" nas configurações, o acúmulo de quadros pulados ou buffer constante é uma indicação de taxa de bits muito alta ou Internet lenta';
    STR_CONTROLS_MULTI_2 = "Adicionar streams: abra a chave do feed de preview e clique em um stream ao vivo";
    ...
    STR_SHOW_FEED_PLAYER_SUMMARY = "Se você não quiser ou seu dispositivo ficar atrasado quando mais de um player estiver ativo, defina como NÃO";
    STR_DISABLED_FEED_PLAYER_MULTI = 'Desativar visualização quando multistream está ativado';
    STR_DISABLED_FEED_PLAYER_MULTI_SUMMARY = 'Por motivos de desempenho, alguns dispositivos podem atrasar com vários players, se você estiver OK para multistream, mas quando o player de pré-visualização e multistream estão ativos, os lags do dispositivo configuram para NÃO';
    ...
    STR_PLAYER_LAG_ERRO = "player incapaz de jogar devido a um problema de conexão de rede";
    STR_PLAYER_ERROR = "player incapaz de jogar, devido ao erro do player";
    ...
    STR_AUTO_REFRESH_SUMMARY = "Quando estiver habilitado, o aplicativo irá atualizar automaticamente uma tela ou uma tela de miniaturas de pré-visualização, a atualização acontece apenas quando a tela é selecionada, se você quiser uma atualização no fundo, habilite o abaixo";
    ...
    STR_ENABLED_MAIN_MULTI = "Habilitar player do canto esquerdo superior ou principal primeiro";
    ...
    STR_SOURCE_CHECK_SUMMARY = 'Quando esta opção está habilitada e você não está usando qualidade automática se o player estiver atrasado, ele mudará para qualidade automática e avisará sobre isso, um atraso do player é, por exemplo, quando o player não consegue jogar por mais de 15 segundos ( o algoritmo é mais complexo do que apenas o tempo, é claro), após essa mudança, o reprodutor voltará automaticamente à fonte quando você iniciar um novo fluxo de vod';
    STR_PLAYER_LAG = 'player está atrasado, qualidade alterada para "Modo automático"';
    STR_PLAYER_SOURCE = 'player está atrasado, qualidade diminuiu';
