...
AGame.StreamGameDiv = 'agame_stream_AGame_';
...
AGame.loadDataRequest = function() {
    try {
        ...
        xmlHttp.onreadystatechange = function() {
            if (xmlHttp.readyState === 4) {
                if (xmlHttp.status === 200) {
                    ...
                    AGame.loadDataError("HTTP Status " + xmlHttp.status + " Message: " + xmlHttp.statusText, xmlHttp.responseText);
                }
            }
        };
        ...
        AGame.loadDataError(e.message, null);
    }
};

AGame.loadDataError = function(reason, responseText) {
...
AGame.createCell = function(row_id, coloumn_id, channel_name, preview_thumbnail, stream_title, stream_game, channel_display_name, viwers, quality) {
    ...
    return $('<td id="' + AGame.Cell + row_id + '_' + coloumn_id + '" class="stream_cell" data-channelname="' + channel_name + '"></td>').html(
        '<img id="' + AGame.Thumbnail + row_id + '_' + coloumn_id + '" class="stream_thumbnail" src="images/video.png"/> \
...
AGame.loadDataSuccessFinish = function() {
    $('#stream_table_a_game').imagesLoaded()
        .always({
            ...
            //if (!AGame.loadingMore) AGame.previewDataStart();
        });
};
...
AGame.loadDataRequestReplace = function() {
    try {
        ...
        AGame.loadDataErrorReplace(e.message, null);
    }
};

AGame.loadDataErrorReplace = function(reason, responseText) {
    AGame.loadingDataTry++;
    if (AGame.loadingDataTry < AGame.loadingDataTryMax) {
        AGame.loadingDataTimeout += (AGame.loadingDataTry < 5) ? 250 : 3500;
        AGame.loadDataRequestReplace();
...
AGame.ScrollHelper = {
    ...
    scrollVerticalToElementById: function(id) {
        ...
        if (id.indexOf(AGame.Thumbnail + '0_') == -1) {
            ...
            $(window).scrollTop(this.documentVerticalScrollPosition() + this.elementVerticalClientPositionById(id) - 0.345 * this.viewportHeight() + 290);
        }
    }
};
