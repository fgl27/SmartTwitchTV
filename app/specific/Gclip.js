...
function Gclip_loadDataSuccess(responseText) {
    ...
    for (var i = 0; i < response_rows; i++) {
        ...
        for (coloumn_id = 0; coloumn_id < Main_ColoumnsCountVideo && cursor < response_items; coloumn_id++, cursor++) {
            ...
            if (Gclip_CellExists(video.tracking_id)) coloumn_id--;
            else {
                Gclip_nameMatrix.push(video.tracking_id);
...
function Gclip_loadDataSuccessReplace(responseText) {
    ...
    for (var i = 0; i < Gclip_blankCellVector.length && cursor < response_items; i++, cursor++) {
        video = response.clips[cursor];
        if (Gclip_CellExists(video.tracking_id)) {
            Gclip_blankCellCount--;
            i--;
        } else {
            Gclip_nameMatrix.push(video.tracking_id);
...
function Gclip_addFocus() {
    Main_addFocusVod(Gclip_cursorY, Gclip_cursorX, Gclip_ids, Main_Gclip, Main_ColoumnsCountVideo, Gclip_itemsCount);
...
function Gclip_removeFocus() {
    Main_removeFocusVod(Gclip_ids, Gclip_cursorY + '_' + Gclip_cursorX);
}
