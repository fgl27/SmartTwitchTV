...
var Gvod_ids = ['gv_thumbdiv', 'gv_img', 'gv_infodiv', 'gv_title', 'gv_streamon', 'gv_duration', 'gv_viwers', 'gv_quality', 'gv_cell', 'gsvempty_', 'vgame'];
    ...
    Main_ready(function() {
        if (!Gvod_status) {
            ...
            if (Gvod_emptyContent) Main_showWarningDialog(STR_NO + (Gvod_highlight ? STR_PAST_HIGHL : STR_PAST_BROA) + STR_FOR_THIS + STR_CHANNEL);
                ...
                Gvod_status = true;
                Gvod_addFocus();
                Main_LazyImgStart(Gvod_ids[1], 9, IMG_404_VIDEO, Main_ColoumnsCountVideo);
            }
            Gvod_loadingData = false;
        } else {
...
function Gvod_loadDataReplace() {
    try {
        ...
        Main_SetItemsLimitReload(Gvod_blankCellCount);

        var offset = Gvod_itemsCount + Gvod_itemsCountOffset;
        if (offset && offset > (Gvod_MaxOffset - 1)) {
            offset = Gvod_MaxOffset - Main_ItemsLimitReload;
            Gvod_ReplacedataEnded = true;
        }
...
function Gvod_addFocus() {
    Main_addFocusVod(Gvod_cursorY, Gvod_cursorX, Gvod_ids, Main_Gvod, Main_ColoumnsCountVideo, Gvod_itemsCount);

    if (Gvod_cursorY > 3) Main_LazyImg(Gvod_ids[1], Gvod_cursorY, IMG_404_VIDEO, Main_ColoumnsCountVideo, 4);
...
function Gvod_removeFocus() {
    Main_removeFocusVod(Gvod_ids, Gvod_cursorY + '_' + Gvod_cursorX);
}
