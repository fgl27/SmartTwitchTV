...
function Screens_HttpResultStatus(resultObj, key) {
    ...
    } else if (resultObj.status === 401 || resultObj.status === 403) { //token expired
...
function Screens_removeFocusFollow(key) {
    ...
    Main_RemoveClass(
        ScreenObj[key].ids[0] + '-1_' + ScreenObj[key].posX,
...
function Screens_KeyUpDownClick(key, y) {
    ...
    if (y > 0) {
        ...
        if (ScreenObj[key].dataEnded ||
            ...
            (ScreenObj[key].BannerCreated &&
                ScreenObj[key].itemsCount === 1 &&
                ScreenObj[key].HasSwitches && ScreenObj[key].posY > -1)) {
