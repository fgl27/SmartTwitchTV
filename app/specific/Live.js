    ...
    for (var i = 0; i < response_rows; i++) {
        ...
        for (coloumn_id = 0; coloumn_id < Main_ColoumnsCountVideo && cursor < response_items; coloumn_id++, cursor++) {
            ...
            if (Live_CellExists(stream.channel.name)) coloumn_id--;
                ...
                row.appendChild(Live_createCell(row_id, row_id + '_' + coloumn_id, stream.channel.name, [stream.preview.template.replace("{width}x{height}", Main_VideoSize),
...
function Live_addFocus() {
    Main_addFocusVideoArray(Live_cursorY, Live_cursorX, Live_ids, Main_Live, Main_ColoumnsCountVideo, Live_itemsCount);
...
function Live_removeFocus() {
    Main_removeFocusVideoArray(Live_cursorY + '_' + Live_cursorX, Live_ids);
}
    ...
    switch (event.keyCode) {
        ...
        case KEY_ENTER:
            if (Main_isExitDialogShown()) {
                ...
                Play_selectedChannel = document.getElementById(Live_ids[8] + Live_cursorY + '_' + Live_cursorX).getAttribute('data-channelname');
