...
-   **Allow to control the Main player of multi-stream separated** Today the same control for resolutions and bitrate applies to all small players including all Multi stream players, it is possible to add a separated option just for the Main one when it is on Main big mode related to [Picture in Picture Quality issue ](https://github.com/fgl27/SmartTwitchTV/issues/138).
...
-   Construct the UI with all possible options, and on its screen, there will be a function that will set with options will be visible, technically what is done today but on a way that is easier to manage
