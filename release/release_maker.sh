...
js_comp_ugf() {
	...
	for i in "${array[@]}"; do
		...
		for x in *.js; do
			...
			uglifyjs "$x" -c -m reserved=['PlayClip_shutdownStream','Play_shutdownStream'] -o "$mainfolder"/"$temp_maker_folder""$x";
...
if [ "$canuglifyjs" == 1 ]; then
	...
	uglifyjs master.js -c -m reserved=['PlayClip_shutdownStream','Play_shutdownStream'],toplevel -o master.js;
fi;
